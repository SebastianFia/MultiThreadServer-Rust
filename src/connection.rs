...
fn get_response(buf_reader: BufReader<&mut TcpStream>) -> ResponseResult<String> {
    ...
    let (filename, status_line) = match &request_first_line[..] {
        "GET / HTTP/1.1" => ("static/home.html", "HTTP/1.1 200 OK\r\n\r\n"),
        "GET /sleep HTTP/1.1" => {
            thread::sleep(Duration::from_secs(5));
            ("index.html", "HTTP/1.1 200 OK\r\n")
        }
        _ => ("static/404.html", "HTTP/1.1 404 Not Found\r\n\r\n"),
    };
